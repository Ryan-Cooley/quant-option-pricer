...
def test_cost_decreases_with_larger_dt():
    ...
    assert cost_dt1 >= cost_dt5 - 1e-6, f"Cost should decrease with larger Δt: {cost_dt1:.3f} vs {cost_dt5:.3f}"
...
def test_csv_output():
    ...
    try:
        ...
        test_args = [
            'run_hedge.py',
            '--S0', '100', '--K', '100', '--T', '0.25', '--r', '0.02', '--sigma', '0.2',
            '--n-paths', '100', '--rebalance-every', '1,5', '--fee-bps', '1.0',
            '--out-csv', csv_path
        ]
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        mock_results = {
            1: HedgeResult(
                ...
                notes={
                    "metrics": {1: {"te_bps": 10.0, "cost_bps": 5.0}},
                    "steps": 252
                }
            ),
            5: HedgeResult(
                ...
                terminal_inventory=np.array([0.1, 0.1]),
                notes={
                    "metrics": {5: {"te_bps": 20.0, "cost_bps": 2.0}},
                    "steps": 252
                }
            )
        }
...
def test_metadata_stamp():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        plot_pnl_histogram(np.array([0.1, 0.2]), {
            "mean": 0.15, "p50": 0.15, "std": 0.05, 
            "sharpe_annualized": 1.0, "p5": 0.1, "p95": 0.2
        }, args, mock_results, plot_path)
