...
def download_log_returns(ticker: str, period: str = '1y',
                         csv_path: str = None, retries: int = 3):
    """
...
def plot_convergence(S0, K, r, sigma, T, steps, path_counts, seed, out_dir):
    ...
    estimates = [monte_carlo_price(S0, K, r, sigma, T, steps, n, seed)
...
def main():
    ...
    mc_price = monte_carlo_price(
        ...
        args.steps, args.paths, args.seed
    )
    ...
    if args.greeks:
        ...
        mc_delta = mc_greek_bump(
            monte_carlo_price, 'S0', eps_S,
            args.S0, args.K, args.r, sigma, args.T,
            args.steps, args.paths, args.seed
        )
        mc_vega = mc_greek_bump(
            monte_carlo_price, 'sigma', eps_sigma,
            args.S0, args.K, args.r, sigma, args.T,
            args.steps, args.paths, args.seed
        )
