...
def time_numpy_call_pricer(n_paths: int, steps: int, *, shared_normals=None) -> tuple[float, float]:
    """
...
def time_numba_call_pricer(n_paths: int, steps: int, *, shared_normals=None) -> tuple[float, float]:
    """
    ...
    Returns:
        Tuple of (time in seconds, computed price), or (np.nan, np.nan) if Numba is not available
    """
    try:
...
def run_bench(path_counts=(10_000, 100_000, 500_000, 1_000_000), steps=252, repeats=3):
    ...
    for n_paths in path_counts:
        ...
        print(f"  Normals: shape={Z.shape}, dtype={Z.dtype}, C-contiguous={Z.flags['C_CONTIGUOUS']}")
        ...
        if numba_times:
            ...
            print(f"  Prices: NumPy={avg_numpy_price:.6f}, Numba={avg_numba_price:.6f}, diff={price_diff:.6f}")
            assert price_diff < 5e-4, f"Price difference {price_diff:.6f} exceeds tolerance 5e-4"
            ...
            print(f"  Prices: NumPy={avg_numpy_price:.6f}, Numba=N/A")
            print(f"  Timing (avg of {repeats} runs):")
            print(f"    RNG generation: {rng_time:.6f}s")
            print(f"    NumPy compute:  {avg_numpy_time:.6f}s")
            print(f"    Numba compute:  N/A")
...
def create_benchmark_plot(df, plot_path="benchmarks/benchmarks.png"):
    ...
    for engine in df_clean['engine'].unique():
        ...
        plt.loglog(engine_data['n_paths'], engine_data['seconds'], 
                  marker='o', linewidth=2, markersize=8, label=engine.capitalize())
    ...
    for engine in df_clean['engine'].unique():
        ...
        if len(engine_data) >= 2:
            ...
            largest_time = engine_data[engine_data['n_paths'] == largest_paths]['seconds'].iloc[0]
            
            # Find corresponding NumPy time for comparison
            if engine == 'numba':
                numpy_time = df_clean[(df_clean['engine'] == 'numpy') & 
                                    (df_clean['n_paths'] == largest_paths)]['seconds'].iloc[0]
